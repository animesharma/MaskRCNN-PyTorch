...
class RandomScale(object):
    ...
    def __call__(self, img, bboxes):
        ...
        #bboxes[:, :4] *= [resize_scale_x, resize_scale_y,
        #                  resize_scale_x, resize_scale_y]
        bboxes[:, :4] = np.multiply(bboxes[:, :4], [resize_scale_x, resize_scale_y,
                          resize_scale_x, resize_scale_y], casting="unsafe")
        ...
        print(f"Image Shape OG: {img_shape}")
...
class RandomTranslate(object):
    ...
    def __call__(self, img, bboxes):
        ...
        orig_box_cords = [max(0, corner_y), max(corner_x, 0), min(
            img_shape[0], corner_y + img.shape[0]), min(img_shape[1], corner_x + img.shape[1])]
...
class RandomShear(object):
    ...
    def __call__(self, img, bboxes):
        ...
        bboxes[:, :4] = np.divide(bboxes[:, :4], [scale_factor_x, 1, scale_factor_x, 1], casting="unsafe")
...
class Resize(object):
    ...
    def __call__(self, img, bboxes):
        ...
        #bboxes[:,:4] *= (scale)
        bboxes[:, :4] = np.multiply(bboxes[:, :4], scale, casting='unsafe')
